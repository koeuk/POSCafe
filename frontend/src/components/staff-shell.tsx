...
function Shell({
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#F5F4FA] text-stone-900">
      {/* Desktop sidebar */}
      <div className="hidden h-screen shrink-0 border-r border-stone-200/70 lg:block">
        {sidebar}
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Top bar */}
        <header className="z-30 flex h-16 shrink-0 items-center gap-3 border-b border-stone-200/70 bg-white/80 px-4 backdrop-blur-xl sm:px-6">
          <button
        ...
        {/* Page content — `bleed` lets a page (e.g. POS) own the full area. */}
        <main
          className={
            bleed
...
/**
 * Shared staff chrome (sidebar + top bar) used across Dashboard, Orders,
 * Products and the POS. Wraps content in RequireAuth. Pass `bleed` for
 * pages that manage their own full-height layout (the POS).
 */
export function StaffShell({
  ...
  return (
    <RequireAuth>
      <Shell bleed={bleed}>{children}</Shell>
    </RequireAuth>
  );
}
