...
export class InitialSchema1785925251672 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`product_variants\` (\`id\` int NOT NULL AUTO_INCREMENT, \`productId\` int NOT NULL, \`size\` varchar(255) NOT NULL, \`price\` decimal(10,2) NOT NULL DEFAULT '0.00', \`sortOrder\` int NOT NULL DEFAULT '0', UNIQUE INDEX \`IDX_274d3685fec36698a94529192f\` (\`productId\`, \`size\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`products\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`description\` varchar(255) NULL, \`price\` decimal(10,2) NOT NULL, \`discountPercent\` int NOT NULL DEFAULT '0', \`image\` varchar(255) NULL, \`gallery\` json NULL, \`isAvailable\` tinyint NOT NULL DEFAULT 1, \`stockMode\` varchar(10) NOT NULL DEFAULT 'count', \`stock\` int NOT NULL DEFAULT '0', \`archivedAt\` datetime(6) NULL, \`categoryId\` int NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`users\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`username\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`email\` varchar(255) NULL, \`avatar\` varchar(255) NULL, \`role\` enum ('admin', 'cashier') NOT NULL DEFAULT 'cashier', \`allowedPages\` text NULL, \`isActive\` tinyint NOT NULL DEFAULT 1, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_fe0bb3f6520ee0469504521e71\` (\`username\`), UNIQUE INDEX \`IDX_97672ac88f789774dd47f7c8be\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`order_items\` (\`id\` int NOT NULL AUTO_INCREMENT, \`orderId\` int NOT NULL, \`productId\` int NOT NULL, \`quantity\` int NOT NULL, \`size\` varchar(255) NULL, \`note\` varchar(255) NULL, \`extras\` json NULL, \`stockSource\` varchar(10) NOT NULL DEFAULT 'stock', \`unitPrice\` decimal(10,2) NOT NULL, \`subtotal\` decimal(10,2) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`stock_movements\` (\`id\` int NOT NULL AUTO_INCREMENT, \`productId\` int NOT NULL, \`delta\` int NOT NULL, \`stockAfter\` int NOT NULL, \`userId\` int NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), INDEX \`IDX_fe8f81647152d8bfb9c7c4e490\` (\`productId\`, \`createdAt\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`app_settings\` (\`id\` int NOT NULL, \`appName\` varchar(255) NOT NULL DEFAULT 'POSCAFE', \`logoUrl\` text NULL, \`khrPerUsd\` int NOT NULL DEFAULT '4100', \`bakongAccountId\` varchar(64) NULL, \`bakongMerchantName\` varchar(25) NULL, \`bakongMerchantCity\` varchar(15) NULL, \`khqrDynamic\` tinyint NOT NULL DEFAULT 1, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
